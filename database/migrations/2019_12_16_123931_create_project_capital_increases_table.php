...
class CreateProjectCapitalIncreasesTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('project_capital_increases', function (Blueprint $table) {
            ...
            $table->string('pub_valueDesc',1000)->comment('专业机构推荐意见');
            ...
            $table->unsignedInteger('pubBailCtrl')->comment('保证金收取方式');
            $table->string('pubBailMemo',1000)->comment('保证金处置方式');
            $table->string('valueDesc',1000)->comment('保证内容');
            ...
            $table->string('dealWayDesc')->comment('遴选方式 其他');
            $table->text('pubDesc')->comment('遴选方案');
            $table->text('spare4')->comment('其他披露事项');
            $table->string('addMoneyPlan',300)->comment('增资方案主要内容');
            $table->text('sellConditions')->comment('增资条件');
