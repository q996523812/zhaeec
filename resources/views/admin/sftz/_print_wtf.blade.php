...
<div class="print">
	<div class="one">
		...
		<div class="call">{{$sftz->wtf}}：</div>
		<div class="content">
			<p>
			{{$sftz->title}}已经圆满结束，贵方以人民币{{$sftz->zbjg_dx}}（小写：¥{{$sftz->zbjg_xx}}元）的价格成交。按照《珠海产权交易中心采购业务委托协议》的约定，贵方需支付本中心交易服务费人民币{{$sftz->zbf_fwf_dx}}（￥{{$sftz->zbf_fwf_xx}}元）。请将上述服务费在 {{$sftz->hk_date}}前汇至下列帐户上：
			</p>
			<p>开户名：{{$sftz->account_name}}</p>
			<p>开户行：{{$sftz->account_bank}}</p>
			<p>帐  号：{{$sftz->account_code}}</p>
